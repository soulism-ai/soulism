...
const run = async () => {
  const path = join(root, 'ci/policies/adapter-framework-boot.policy.json');
  const policy = JSON.parse(await readFile(path, 'utf8')) as FrameworkBootPolicy;
  ...
  if (failures.length > 0) {
    console.error(`Adapter framework boot policy validation failed: ${failures.join(',')}`);
    process.exit(1);
  }

  console.log('Adapter framework boot policy validation passed.');
};
