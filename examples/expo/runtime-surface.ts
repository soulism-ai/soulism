...
export const startExpoRuntimeSurface = async (gatewayUrl: string): Promise<RuntimeSurfaceServer> => {
  const server = createServer(async (req, res) => {
    const method = req.method || 'GET';
    const path = (req.url || '/').split('?')[0];

    if (method === 'GET' && path === '/health') {
      sendJson(res, 200, { ok: true, runtime: 'expo' });
      return;
    }

    if (method === 'POST' && path === '/parity/run') {
      const body = await readJson(req);
      const adapterId = String(body.adapterId || 'expo-adapter');
  ...
  if (!address || typeof address === 'string') throw new Error('expo_runtime_surface_bind_failed');
  return {
...
if (process.argv[1] && process.argv[1].endsWith('runtime-surface.ts')) {
  ...
  void startExpoRuntimeSurface(gatewayUrl)
    .then((running) => {
      console.log(`expo_runtime_surface_ready:${running.url}`);
    })
    .catch((error) => {
