...
import { resolveQuickstartConfig, type ResolveConfigInput } from './config.js';
import {
  buildPolicyRequest,
  formatBudget,
  renderResult,
  type QuickstartRunResult,
...
const toToolResult = (tool: string, status: number, ok: boolean, contentType: string, body: string, url: string): QuickstartRunResult['tool'] => ({
  status,
  ok,
  contentType,
  body,
...
const callWebfetchTool = async (context: ToolContext, targetUrl: string, method = 'GET'): Promise<QuickstartRunResult['tool']> => {
...
): Promise<QuickstartRunResult['tool']> => {
...
const resolveToolExecutor = (tool: ToolChoice, context: ToolContext, params: QuickstartParams): Promise<QuickstartRunResult['tool']> => {
...
export const runQuickstartFlow = async (params: QuickstartParams = {}): Promise<QuickstartRunResult> => {
  const config = resolveQuickstartConfig(params.config);
