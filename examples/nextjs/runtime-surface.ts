...
const runProbe = async (gatewayUrl: string, adapterId: string): Promise<RuntimeProbeResult> => {
  ...
  if ((allow.body.state as string) in policyStateCounts) {
  ...
  if ((confirm.body.state as string) in policyStateCounts) {
  ...
  if ((deny.body.state as string) in policyStateCounts) {
    policyStateCounts[deny.body.state as keyof typeof policyStateCounts] += 1;
  }

  const personas = await callJson(`${gatewayUrl}/personas`, {
    method: 'GET',
    headers: baseHeaders
  });
...
export const startNextjsRuntimeSurface = async (gatewayUrl: string): Promise<RuntimeSurfaceServer> => {
  const server = createServer(async (req, res) => {
    const method = req.method || 'GET';
    const path = (req.url || '/').split('?')[0];

    if (method === 'GET' && path === '/health') {
      sendJson(res, 200, { ok: true, runtime: 'nextjs' });
      return;
    }

    if (method === 'POST' && path === '/parity/run') {
      const body = await readJson(req);
      const adapterId = String(body.adapterId || 'nextjs-adapter');
  ...
  if (!address || typeof address === 'string') throw new Error('nextjs_runtime_surface_bind_failed');
  return {
...
if (process.argv[1] && process.argv[1].endsWith('runtime-surface.ts')) {
  ...
  void startNextjsRuntimeSurface(gatewayUrl)
    .then((running) => {
      console.log(`nextjs_runtime_surface_ready:${running.url}`);
    })
    .catch((error) => {
